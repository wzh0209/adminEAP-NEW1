    ...
    public void notify(DelegateTask delegateTask) {
        ...
        if(EVENTNAME_COMPLETE.equals(eventName)){
             ...
             if("true".equalsIgnoreCase(delegateTask.getVariable("approved").toString())){
            	 if("rl".equals(taskDefinitionKey)){
                	 zf.setLeaveStatus("7");
                	 zf.setRlyj(suggestion);
                 }else if ("cw".equals(taskDefinitionKey)) {
                	 zf.setLeaveStatus("10");
                	 zf.setCwyj(suggestion);
                 }else if ("xz".equals(taskDefinitionKey)) {
                	 zf.setLeaveStatus("30");
                	 zf.setBgsyj(suggestion);
                 } else if ("boss".equals(taskDefinitionKey)) {
                	 zf.setLeaveStatus("40");
                	 zf.setZjlyj(suggestion);
                	 //删除用户
