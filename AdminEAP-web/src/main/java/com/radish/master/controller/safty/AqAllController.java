...
import com.radish.master.entity.safty.AqAll;
 ...
 */
@Controller
@RequestMapping("/aqAll")
public class AqAllController {
    ...
    public String listIndex(HttpServletRequest request) {
		...
		return "safetyManage/aqAll/mod"+type+"/addIndex";
    }
    ...
    public String addWj(HttpServletRequest request) {
		...
		List<Options> fileTypes = baseService.findMapBySql("select names data,names value  from tbl_aqfileselects "
				+ "where type='"+type+"' ", new Object[]{}, new Type[]{StringType.INSTANCE}, Options.class);
		request.setAttribute("fileType", JSONArray.toJSONString(fileTypes));
        return "safetyManage/aqAll/mod"+type+"/edit";
    }
...
//		request.setAttribute("fileType", JSONArray.toJSONString(fileTypes));
        ...
        return "safetyManage/aqAll/mod"+type+"/look";
    }
	...
	public Result load(String id){
		...
		AqAll  jd= baseService.get(AqAll.class, id);
		 Result r = new Result();
		 r.setData(jd);
		 r.setCode(wjid);
		return r;
    ...
    public String scWj(HttpServletRequest request) {
		...
		String islook = request.getParameter("islook");
		request.setAttribute("islook",islook);
		return "safetyManage/aqAll/mod18/query_file";
    }
    
    @RequestMapping("saveWj")
    @ResponseBody
    public Result saveWj(HttpServletRequest request,AqAll cr){
    	Result r = new Result();
    	...
    	if(cr.getId()==null){
    		...
    		AqAll c = baseService.get(AqAll.class, cr.getId());
    		c.setProid(cr.getProid());
    		c.setFileType(cr.getFileType());
    		c.setCol1(cr.getCol1());
    		c.setCol2(cr.getCol2());
    		c.setCol3(cr.getCol3());
    		c.setCol4(cr.getCol4());
    		c.setCol5(cr.getCol5());
    		c.setCol6(cr.getCol6());
    		c.setCol7(cr.getCol7());
    		c.setCol8(cr.getCol8());
    		c.setNotes(cr.getNotes());
    ...
    public Result deleteWj(String id,HttpServletRequest request) {
    	...
    	AqAll cr = baseService.get(AqAll.class, id);
    ...
    public FileResult getPreivewSettings(List<CheckFileAQ> fileList,HttpServletRequest request){
        ...
        for (CheckFileAQ sysFile : fileList) {
            ...
            //如果其他文件可预览txt、xml、html、pdf等 可在此配置
        	 //如果其他文件可预览txt、xml、html、pdf等 可在此配置
        	String dirPath = request.getContextPath() + "/aqAll/showImage?imageID=" + sysFile.getId();
